...
const SpellForm = ({ onSpellAdded }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (value.length > 30) {
      setError("Не більше 30-ти символів");
      return;
    } else {
      setError("");
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="spell-form">
      ...
      <label>
        Рівень ({spellConfig.spellLevels.min}-{spellConfig.spellLevels.max}):
      </label>
      <input
        type="number"
        name="level"
      ...
      <label>
        💠 Кількість дій ({spellConfig.actions.min}-{spellConfig.actions.max}):
      </label>
      <input
        type="number"
        name="actions"
      ...
      <div className="duration-group">
          ...
          {spell.duration.unit !== "until_short_rest" &&
            spell.duration.unit !== "until_long_rest" && (
              <input
                ...
                min="0"
                required={
                  spellConfig.durationUnits.find(
                    (u) => u.value === spell.duration.unit,
                  )?.requiresValue
                }
              />
            )}
        ...
        {spellConfig.durationUnits.find((u) => u.value === spell.duration.unit)
          ?.requiresCustomUnit && (
          <input
            type="text"
